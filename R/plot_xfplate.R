...
#' @export
#' @importFrom ggplot2 ggplot aes geom_line geom_ribbon geom_hline geom_vline annotate 
#' scale_x_continuous scale_y_discrete labs %+replace% theme_light theme rel scale_y_continuous
...
sketch_assimilate_raw <- function(my_df, param = "O2_mmHg"){
  ...
  my_df %>% 
    ...
    dplyr::select(plate_id, measurement, well, group, emission = all_of(param)) %>% 
    dplyr::slice(which.min(measurement), .by = c(plate_id, well)) %>% 
    dplyr::mutate(group_id = case_when(group == "Background" ~ "background",
                                      .default = "sample")) %>% 
    ggplot(aes(x = emission, y = plate_id, group = plate_id)) + 
        ...
        geom_vline(xintercept = targetEMS, linetype = "dashed", color = "#D16103")+
        scale_discrete_manual(aesthetics = 
                            "point_color", 
...
sketch_assimilate_rate <- function(my_df, 
  ...
  my_df %>% 
    ...
    dplyr::mutate(group_id = paste0(plate_id, "_", group)) %>% 
    relevel_group_names() %>% 
    ggplot(aes(x = group, y = rate, color = as.factor(interval))) + 
      colorspace::scale_color_discrete_divergingx(palette = "Geyser", 
                                                  rev= TRUE,
                                                  name = "interval")+
      geom_jitter()+
      labs(x = "",
      ...
      theme_BF()+
      theme(axis.text.x = 
              element_text(size = rel(0.8), 
                           angle = 50, vjust = 0.9, hjust = 1))+
      facet_wrap(~plate_id)
...
# ggplot functions ------------------------------------
  ...
  plot_ribbon_per_meas_and_group <- function(df, var, y_title){
    ...
    theme_ribbon <- function(){
      theme_light(base_size = 20) %+replace%
        theme(panel.grid.minor.x = element_blank(),
              ...
              plot.title = element_text(hjust = 0, color="black", size=rel(1)),
    ...
    label_inj <- df %>% pull(injection) %>%  
      unique() %>% 
      tail(-1)
    
    quantiles_df<- df %>%
      select(everything(), param = all_of(var)) %>%
      summarise(q = list(quantile(param)), 
    ...
    plot <- quantiles_df %>%
      mutate(whisker_up = X100,
             whisker_down = X0) %>%
      ggplot(aes(x = time_wave, group = group))+
        geom_ribbon(aes(ymin = X50 - (X50-X25), ymax = X50 + (X75-X50), fill = group), alpha = 0.5)+
        geom_line(aes(y = X50, color = group),linewidth = 1.5, linetype = "solid")+
        geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.4, color = "#D16103")+
        geom_vline(xintercept = xintercept_inj,
                 color = "grey40",
                 linetype = "dashed",)+
        annotate("text", 
        ...
        colorspace::scale_color_discrete_divergingx(palette = "Geyser", rev= TRUE)+
        colorspace::scale_fill_discrete_divergingx(palette = "Geyser", rev= TRUE)+
        labs(y = y_title,
             x = "time (minutes)")+
  ...
  plot_line_per_well <- function(df, var, y_title){
    ...
    plot <- df %>% 
      select(everything(), param = all_of(var)) %>%
      ggplot(aes(x = time_wave, y = param, 
                group = well,
                color = group)) +
