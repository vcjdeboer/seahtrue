...
#data
data <- readRDS("scripts/data/my_data_PBMC.Rda")

#script
processed <- data %>% 
  mutate(pr_raw_data = map2(raw_data, 
                            assay_info,
                            ~mutate(.x,
  ...
  mutate(ocr_without_meta = map2(pr_raw_data,
                    assay_info,
                    ~{.x %>%
                        split(f = as.factor(.$well)) %>% 
                        map2(rep(list(.y), length(.)), 
                             ~{add_extraTicks_new_2(.x, "O2_M_mmHg") %>% calculate_OCR_new(.y)}) %>% 
                        bind_rows(.id = "well")})) %>% 
  mutate(ocr = map2(raw_data,
                    ocr_without_meta,
                    ~{.x %>% 
