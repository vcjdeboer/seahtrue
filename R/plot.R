...
plot_raw_BKGD <- function(var, xf_raw_pr, O2_targetEmission, pH_targetEmission, flnme){
  ...
  gg_plot = ggplot2::ggplot(df, ggplot2::aes(x=minutes, y=emission, color = well))+
    ggiraph::geom_point_interactive(mapping = ggplot2::aes(colour  = well, tooltip = well, data_id = well), alpha = 0.5, size = 3)+
...
plot_raw_BKGD_means <- function(var, xf_raw_pr, O2_targetEmission, pH_targetEmission, flnme){
  ...
  gg_plot = ggplot(mean_O2_em_corr, ggplot2::aes(x = minutes, y = O2_em_corr_mean))+
    ggiraph::geom_point_interactive(mapping = ggplot2::aes(x = minutes, y = O2_em_corr_mean, data_id = O2_em_corr_mean), alpha = 0.5, size = 3)+
    ggplot2::geom_errorbar(mapping = ggplot2::aes(x = minutes,
                                         y = O2_em_corr_mean,
...
plot_BKGD_auc_facet <- function(var, auc_var, xf_raw_pr, O2_targetEmission, pH_targetEmission, file_name){
  ...
  #' @return Area under curve background.
  #' #'
  ...
  #'
  #' @examples get_BKGD_auc(xf_plate_pr$raw_data[[1]], "O2_em_corr", 12500)
  #'
  ...
  make_raw_em_corr_BKGD_plot <- function(AUC_bkgd, var, auc_var, flnme){
    logger::log_info("Plot fluorescence emission (Au) ")

    gg_plot <- ggplot2::ggplot(data = AUC_bkgd,
                      mapping = ggplot2::aes(x = measurement, y = BKG_auc))+
      ggplot2::geom_smooth(method = 'loess', formula = 'y~x', se=F, color = "#293352")+
      ggplot2::geom_col(alpha = 0.5, ggplot2::aes(fill = well)) +
      ggplot2::scale_fill_manual(values=custom.col)+
      ggplot2::facet_wrap(~well, nrow = 2)+
      theme_maxTick()+
      ggplot2::labs(title = paste0(var, " - ", auc_var),
           subtitle = flnme,
           x = "measurement",
  ...
  df_forPlot <- get_BKGD_auc(xf_raw_pr, var, targetEMS) %>% dplyr::select(well, measurement, BKG_auc = .data[[auc_var]])
...
map_groupMaker <- function(XFe96data, flnme){
  ...
  theme_htmp<- function(){
    ggplot2::theme_bw(base_size = 60) %+replace%
      ggplot2::theme(panel.grid.minor.x = element_blank(),
            panel.grid.major.x = element_blank(),
            panel.grid.minor.y = element_blank(),
            panel.grid.major.y = element_blank(),
  ...
  wholeWell_RC <- wholeWell %>% tidyr::separate(well2, into = c("row", "column"), sep = 1, convert = TRUE)
  wholeWell_measurement <- dplyr::filter(wholeWell_RC, measurement == 1)

  nb.cols <- nlevels(as.factor(wholeWell$group))
  groupColors3 <- grDevices::colorRampPalette(brewer.pal(8, "BrBG"))(nb.cols)
  #map
  gg_plot <- ggplot2::ggplot(data = wholeWell_measurement, ggplot2::aes(x = column, y = row)) +
    ggiraph::geom_tile_interactive(ggplot2::aes(fill = as.factor(group)), color = "grey50",show.legend = TRUE)+
    ggplot2::scale_fill_manual(values= groupColors3 )+
    ggplot2::labs(title = flnme)+
    ggplot2::scale_x_continuous(limits= c(0.5, 13),
                       breaks = c(1:12),
                       position = "top")+
...
plot_group_emissions <- function(var, xf_raw_pr, O2_targetEmission, pH_targetEmission, flnme){
  ...
  df <- xf_raw_pr %>% dplyr::select(measurement, well, group, timescale, minutes, tick, emission = .data[[var]] )
  df <- df[!is.na(df$emission), ]
  ...
  mean_O2_em_corr <- df %>% dplyr::group_by(group,measurement) %>%
    dplyr::summarize(m_em = mean(emission),
              sd_em = sd(emission))
  ...
  gg <- ggplot2::ggplot(data = df,
    ...
    ggplot2::labs(title = var,
         ...
         x = "measurement",
         y = "emission (AU)")+
    ggplot2::theme(plot.title = element_text(hjust = 0.5, size = 5)) +
...
make_range_plot <- function(var, df, flnme){
  ...
  get_range <- function(var, xf_raw_pr){
    ...
    df<- xf_raw_pr %>% dplyr::filter(!group == "Background") %>%
      dplyr::select(measurement, well, group,timescale, tick, minutes, interval, param = .data[[var]])
    df <- df[!is.na(df$param), ]

    temp_df <- NULL
    temp_range <- tidyr::tibble(group = 0, measurement = 0, interval = 0, well = 0, min = 0, max = 0, range =0)
    range_allGroups <- NULL
    wellList <- unique(as.vector((df$well)))
    measurementList <- unique(as.vector((df$measurement)))
    for (g in wellList){
      for (k in measurementList){
        temp_df <- df %>% dplyr::filter(measurement == k & well ==g)
    ...
    mean_range_measurement <- range_allGroups %>% dplyr::group_by(group,measurement) %>%
      dplyr::summarize(m_min = mean(min),
                m_max = mean(max),
                m_range = mean(range))
  ...
  injection_maxes <- df %>% dplyr::group_by(interval) %>% dplyr::summarize(max = max(measurement))
  ...
  gg_plot <- ggplot2::ggplot(data = mean_range)+
    ggplot2::geom_crossbar(ggplot2::aes(ymin = m_min, ymax = m_max, x = measurement, y = m_min),
                  fill = "#293352", color = NA, alpha = 0.5, fatten = 0)+
    ggplot2::geom_hline(yintercept = target, linetype = "dashed", color = "#D16103")+
    ggplot2::geom_vline(xintercept = injection_xpoints, linetype = "dotted", color = "#D16103", size = 0.2)+
    theme_maxTick()+
    ggplot2::scale_x_continuous(limits = c(0,max(mean_range$measurement)+1),
...
plot_allWells_firstTicks <- function(var, xf_raw_pr, O2_targetEmission, pH_targetEmission, flnme, well_letter){
  ...
  if(grepl('[A-Z]', well_letter) == TRUE){
    ...
    gg_plot <- ggplot2::ggplot(data = df,
                      mapping = ggplot2::aes(x = measurement, y = emission, group = measurement, tooltip = well, data_id = well))+
      ggiraph::geom_line_interactive(ggplot2::aes(x = measurement, y = emission, group = well), alpha = 0.5, color = "#4E84C4")+
      ggiraph::geom_point_interactive(ggplot2::aes(x = measurement, y = emission, group = well), alpha = 0.8, color = "#4E84C4")+
      ggiraph::geom_hline_interactive(yintercept = targetEMS, linetype = "dashed", color = "#D16103")+
      ggplot2::scale_x_continuous(limits = c(0,max(df$measurement)+1),
                                  breaks = seq(1, max(df$measurement), by = 1),
                                  expand = c(0, -0.3))+
