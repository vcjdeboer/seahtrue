...
get_xf_raw <- function(fileName){
  ...
  xf_raw <- read_excel(fileName, sheet = "Raw")
  return(xf_raw)
}

# get_xf_norm -------------------------------------------------------------

get_xf_norm <- function(fileName){
...
get_xf_flagged <- function(fileName){
  
  #read excel file using tidyxl
  x <- xlsx_cells(fileName, "Assay Configuration")
  formats <- xlsx_formats(fileName, "Assay Configuration")
...
get_xf_rate <- function(fileName){
  #first item is table, second ite is background_corrected logical
  xf_rate_list <- get_originalRateTable(fileName)
}

# get_xf_buffer -----------------------------------------------------------

get_xf_buffer <- function(fileName){
...
get_xf_pHcal <- function(fileName){
...
get_xf_O2cal <- function(fileName){
  pH_calibration <- get_platelayout_data(fileName,
                                         my_sheet = "Calibration",
                                         my_range = "B7:N15",
                                         my_param = "O2_cal_em")
...
# get_xf_in()j --------------------------------------------------------------
get_xf_inj <- function(fileName, injscheme = "HAP"){
  ...
  info_sh<-read_excel(fileName, sheet = "Operation Log")
...
# get_assay_info_new() ----------------------------------------------------
get_xf_assayinfo <- function(fileName, 
                             date_style = "US", 
                             instrument = "XFe96",
                             norm_available,
...
# read_xfplate() -------------------------------------------------------
read_xfplate <- function(fileName) {
  
  #read data
  xf_raw <- get_xf_raw(fileName)
  xf_rate <- get_xf_rate(fileName) #outputs list of 2
  xf_norm <- get_xf_norm(fileName) #outputs list of 2
  xf_buffer <- get_xf_buffer(fileName)
  xf_inj <- get_xf_inj(fileName)
  xf_pHcal <- get_xf_pHcal(fileName)
  xf_O2cal <- get_xf_O2cal(fileName)
  xf_flagged <- get_xf_flagged(fileName)
  xf_assayinfo <- get_xf_assayinfo(fileName, 
