...
for (rda_file in filepath){
  ...
  local({
    ...
    test_that("type integer", {
      ...
      expect_type(xf_plate_pr$rate_data[[1]][1]$measurement, "double") # actually an integer
    })
  ...
  test_that("type logical or double", {
      ...
      if (all(is.na(xf_plate_pr$rate_data[[1]][9]$cell_n)) == TRUE) {
        expect_type(xf_plate_pr$rate_data[[1]][9]$cell_n, "logical")
      } else {
        expect_type(xf_plate_pr$rate_data[[1]][9]$cell_n, "double") #actually integer
      }
